...
```javascript
{
  userId: number, // primary key
    ...
    role: string // not null, unique // two types : finance mgr and employee; employee cannot access mgr endpoint
  } // not null
}
```
...
```javascript
{
  ...
  status: number { //Status possibilities are `Pending`, `Approved`, or `Denied`.
    statusId: number, // primary key
    status: string // not null, unique
  }, // foreign ey -> ReimbursementStatus, not null
  type: ReimbursementType { //Type possibilities are `Lodging`, `Travel`, `Food`, or `Other`
    typeId: number, // primary key
    type: string, // not null, unique
  }// foreign key -> ReimbursementType
}
```
